...
## Function definition
Mpsr4 <- function(tseries, nEp = 2, span = 3) {
  # Returns the multivariate potential scale reduction (mpsr) factor assessing contemporaneous correlation
  # Argument: 
  #   tseries
  ...
  # ---------------------------------------------------------------
  #Correlation product
  pairs <- combn(ncol(per_dev), 2) #define all unique pairs
  
  per_dev <- vapply(seq_len(ncol(pairs)), function(m) { #multiply contemporaneous values for each pair
    per_dev[, pairs[1, m]] * per_dev[, pairs[2, m]]
  }, FUN.VALUE = numeric(N))
  ...
  # PSR^P value
  Mpsr4 <- sqrt((median(epoch_L) - 1) / median(epoch_L) + ((nEp + 1) / nEp) * lambda1)
  return(Mpsr4)
}
