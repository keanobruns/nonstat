...
## Function definition
Mpsr5 <- function(tseries, nEp = 2, span = 3, lag = 1) {
  # Returns the multivariate potential scale reduction (mpsr) factor assessing lagged correlation
  # Argument: 
  #   tseries
  ...
  #Lag-1 Correlation product
  pairs <- combn(ncol(per_dev), 2, simplify = FALSE) #unique pairs
  
  per_dev <- do.call(cbind, lapply(pairs, function(ix) {
    i <- ix[1]; j <- ix[2]
    x_lag <- ave(per_dev[, i], index, FUN = function(u) c(rep(NA_real_, lag), head(u, -lag)))
    y_lag <- ave(per_dev[, j], index, FUN = function(u) c(rep(NA_real_, lag), head(u, -lag)))
    cbind(
      #per_dev[, i] * c(NA_real_, per_dev[-n, j]),  # x_t * y_{t-1}
      #c(NA_real_, per_dev[-n, i]) * per_dev[, j]   # x_{t-1} * y_t
      per_dev[, i] * y_lag, 
      x_lag * per_dev[, j]
    )
  }))
  ...
  # PSR^P value
  Mpsr5 <- sqrt((median(epoch_L) - 1) / median(epoch_L) + ((nEp + 1) / nEp) * lambda1)
  return(Mpsr5)
}
